...
class RenameHandleToDoHandleRecipeTest implements RewriteTest {

  Parser.Builder parser = JavaParser.fromJavaVersion().classpath("cqs-core", "cqs-spring");

  @Override
  public void defaults(RecipeSpec spec) {
    spec.recipe(new RenameHandleToDoHandleRecipe())
        .parser(parser)
    ...
    rewriteRun(
        java(
            """
              ...
              import eu.prismacapacity.cqs.spring.cmd.SpringCommandHandler;

              class FooBar extends SpringCommandHandler<Command> {
                  @Override
                  public void handle(Command cmd) {
                  }

                  @Override
                  public void verify(Command cmd) {
                  }
              }
              """,
            """
